 ...
 *
 */
public class SuffixTreeMISTAKE {
    ...
    public void build(String word) {
        ...
        while (index < chars.length) {// 循环建立后缀
            ...
            char w = chars[currentIndex];// 当前的后缀字符
            ...
            if (reminder == 0) {
                ...
                node.totallength = splitNode.totallength+activePoint.length;
                ...
                newNode.totallength = node.totallength;
     ...
     */
    private void innerSplit(char[] chars, int currentIndex, Node prefixNode) {
        // 此处计算剩余待插入的后缀的开始位置，例如我们需要插入三个后缀(abx,bx,x)，已处理了abx，则还剩余ba和x，则下面计算的位置就是b的位置
        int start = currentIndex - reminder + 1;

        this.print();// 打印
        System.out.println();
        System.out.println("当前插入后缀：" + String.copyValueOf(chars, start, currentIndex - start + 1) + "========");
        ...
        System.out.println("activePoint.length = " + activePoint.length);
        // int dealStart = start + activePoint.point.chars.length + activePoint.length;
        int dealStart = start + activePoint.point.chars.length + activePoint.point.totallength;
        // 从dealStart开始查找所有后缀字符是否都存在(相对与活动点)
        System.out.println(dealStart + "," + currentIndex);
        if (currentIndex-dealStart+1>0) System.out.println("查找的后缀： " + String.copyValueOf(chars, dealStart, currentIndex - dealStart + 1) + "========");
        System.out.println("activePoint.point.chars.length = "+ activePoint.point.chars.length);
        for (int index = dealStart; index <= currentIndex; index++) {
            ...
            if(null==activePoint.index){// 如果activePoint.index==null，说明没有找到活动边，那么只需要在活动节点下插入一个节点即可
                ...
                node.totallength = activePoint.point.totallength + activePoint.point.chars.length;
                Node child = activePoint.point.child;
                if(null==child){
                    activePoint.point.child = node;
                }else{
    ...
    private boolean find(char w) {
        ...
        if (current != null) System.out.println("current : " + current.chars[0]);
        ...
        if (null == current) {// current==null 无活动边，则从活动点的子节点开始查找
            ...
            /*
            if (activePoint.length >= current.chars.length) {
                activePoint.point = current;
                activePoint.index = null;
                activePoint.length -= current.chars.length;
                return find(w);
            }
            */
            if (activePoint.length >= current.chars.length) {
            ...
            if (current.chars[activePoint.length] == w) {// 有活动边，则在活动边上查找
                activePoint.length++;
                exist = true;
    ...
    public static void main(String[] args) {
        ...
        //tree.build("abcdbcdxcdxgabcdbcdxcdxg$");
        tree.build("abcdabcxbcx$");
        tree.print();

        // 查找子串
        String word = "abc";
        System.out.println(tree.select(word));
        System.out.println(tree.select("bcx$"));
        */

        SuffixTreeMISTAKE tree = new SuffixTreeMISTAKE();
        tree.build("cdddcdca$");
        tree.print();
    }
}